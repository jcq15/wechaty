...
class cyjl_server:
    ...
    def __init__(self):   
        ...
        if len(cyjl_server.data) == 0:
            with open("/home/wechat/wechatbot/coal_dict.txt", "r", encoding='gbk') as f:
                counter = 0
                for line in f:
                    content = line.split("\t")
                    #word = content[0]
                    ...
                    if not content[2] in cyjl_server.data:
                        cyjl_server.data[content[2]] = (content[1].split(' '))[:-1]
                        counter += 1
    ...
    def start(self):
        while True:
            word = random.sample(cyjl_server.data.keys(), 1)[0]
            print(word[-1])
    ...
    def get_start_with(self, last_word):
        result = [word for word in cyjl_server.data if cyjl_server.data[word][0] == cyjl_server.data[last_word][-1]]
    ...
    def judge(self, answer, username, nickname):
        if answer in self.result:    # 这该死的群友，数理基础竟然如此扎实
            ...
            self.result = self.get_start_with(answer)
            return 1
        return 0
...
# 根据文本创建gif
def create_gif(text, filename='test.gif', imgsize=(600, 200), bgcolor=(255, 255, 255),
           fgcolor=(102, 8, 116), fontfile='/home/wechat/wechatbot/fonts/STXINGKA.ttf', fontsize=60, duration=200, loop=0):
    ...
    def handle(self, data):
        ...
        if data['mentionSelf']:
            ...
            if self.status == 'cyjl':
                ...
                elif len(text) > 0 and cyjl_server.data[self.cyjl.now][-1] in pinyin(text, style=Style.NORMAL)[0]:                 # 第一个字对上了
                    result = self.cyjl.judge(text, userid, usernick)
                    if result == 1:             # 还真接上了
                        ...
                        return '%s，不对不对！你说的什么玩意，【%s】接不上！' % (usernick, text)
            ...
            # 未正式上线的测试功能
                ...
                content = text[4:]
                filename = '/home/wechat/wechatbot/images/' + str(uuid.uuid1()) + '.gif'
                create_gif(content, filename=filename)
